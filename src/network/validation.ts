...
export function parseServerMessage(raw: string): ServerMessage | null {
  ...
  for (const worm of message.worms) {
    ...
    if (!['score', 'spawnProtection', 'speedTicks', 'chompTicks', 'wideTicks', 'multiplierTicks', 'combo', 'facePhase'].every(key => number(worm[key], 0, 999999999))) return null;
    ...
    if (!number(worm.wideBlend, 0, 1)) return null;
  ...
  if (!isRecord(status) || !['score', 'size', 'multiplier', 'multiplierSeconds', 'speedSeconds', 'chompSeconds', 'wideSeconds', 'combo'].every(key => number(status[key], 0, 999999999))) return null;
