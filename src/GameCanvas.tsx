      ...
      if (!document.hidden) {
        ...
        if (current.state === 'playing' || (online && current.state === 'paused')) {
          ...
          if (time - lastPublished > 100) {
            ...
            if (status) {
              const statusKey = `${status.score}:${status.size}:${status.sizeRank}:${status.multiplier}:${status.multiplierSeconds}:${status.speedSeconds}:${status.chompSeconds}:${status.wideSeconds}:${status.combo}:${status.activeCount}:${status.humanCount}:${status.botCount}:${status.connectedCount}:${status.leaderboard[0]?.score ?? 0}:${status.leaderboard.reduce((m, e) => Math.max(m, e.size), 0)}`;
